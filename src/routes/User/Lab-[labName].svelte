<!--
    Author: Nate Fabian
    Date: 2/12/22
    Purpose: Collect student answers to lab questions and submit to APIs/Labs/postLabAnswers
-->
<script lang="ts">
	...
	async function Submit() {
		...
		if (checkIfAllAnswered()) {
			let fd = new FormData();
			fd.set('Email', auth.currentUser.email);
			fd.set('LabName', labName);
			fd.set('AnswersArray', JSON.stringify(submittedAnswer));
